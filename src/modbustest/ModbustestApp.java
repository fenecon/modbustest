			...
			if (ports != null) {
				for (SerialPort port : ports) {
					...
					if (isWindows) {
						...
						continue; // ignore ttyS0
					} else if (port.getSystemPortName().equals("ttymxc0")) {
						continue; // ignore ttyS0
					} else {
					...
					for (Device device : devices) {
						Log.info(Log.HIGH_INTENSITY + Log.CYAN + "- Trying to find [" + device.getName() + "]"
								+ Log.ANSI_RESET);
						if (device.detectDevice()) {
							Log.info("Detected " + Log.HIGH_INTENSITY + Log.GREEN + "[" + device.getName() + "]"
									+ Log.ANSI_RESET);
